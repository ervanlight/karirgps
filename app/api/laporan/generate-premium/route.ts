...
// Maksimal 60 detik di Vercel Hobby, cukup untuk Gemini 2.5 Pro
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (existingReport?.laporan_siswa && !(existingReport.laporan_siswa as any)._error) {
      return NextResponse.json({ status: 'ok', laporan_siswa: existingReport.laporan_siswa })
    }

    console.log('🚀 Menjalankan AI Generation Premium (Decoupled)...')
